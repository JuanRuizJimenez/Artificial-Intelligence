...
public class AStar : MonoBehaviour
{
    ...
    private void Setup()
    {
        ...
        int W, H;
        W = GM.GetComponent<MapGenerator>().xSize;
        H = GM.GetComponent<MapGenerator>().zSize;
		...
		while (abiertos.Count > 0 && !FOUND)
        {
            ...
            foreach (coords c in adyacentes)
            {
                ...
                if (nodeMatrix[c.x_, c.z_] == null)
                {
                    ...
                    // si la casilla a la que se le asocia dicho nodo no es una roca
                    // lo metemos en abiertos
                    if (table[ady.i_, ady.j_].cell != Cells.rocks)
                    {
                ...
                else
                {
                    ...
                    if (table[ady.i_, ady.j_].cell == Cells.Empty)
                        weight = 1;
                    else if (table[ady.i_, ady.j_].cell == Cells.Water)
                        weight = 2;
                    else if (table[ady.i_, ady.j_].cell == Cells.Mud)
                        weight = 4;
    ...
    public int calculateG(Node n, box[,] table)
    {
        int weight = 0;

        if (table[n.i_, n.j_].cell == Cells.Empty)
            weight = 1;
        else if (table[n.i_, n.j_].cell == Cells.Water)
            weight = 2;
        else if (table[n.i_, n.j_].cell == Cells.Mud)
            weight = 4;
    ...
    public void calculatePath(Node n, int origX, int origZ)
    {
        ...
        foreach(Node aux in RESULT)
        {
			if(aux != RESULT[RESULT.Count - 1]) {

				int zDir = 0;

				if(RESULT[count].i_ < aux.i_)
				{
					zDir = 90;
				}

				else if (RESULT[count].i_ > aux.i_)
				{
					zDir = -90;
				}

				else if (RESULT[count].j_ > aux.j_)
				{
					zDir = 180;
				}

				FindObjectOfType<MapGenerator>().createArrows(aux.i_, aux.j_, zDir);

				count++;
			}
        }

		FindObjectOfType<HUDControllScript>().setSteps(RESULT.Count - 1);
	}
